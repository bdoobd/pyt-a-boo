...
def run(amount, trade_open=False):
    ...
    asset = Asset('usdt', type='test')

    print(asset.getTicker().head(20))
    ...
    if choice == 'Y':
        ...
        if buy_order:
            ...
            sell_price = asset.getFilteredPrice(receipt_price * 1.02)
            sell_order = trade.createLimitSellOrder(
                qty=receipt_qty, price=sell_price)
            # sell_order = True

            if (sell_order):
                print(
                    f'Заказ на продажу установлен. Количество {receipt_qty}, стоимость {sell_price}')

            # TODO: Что делать дальше?

            # FIX: При перезапуске скрипта надо проверить наличие отктытого ордера и продолжить отслеживать
            # ==========================================
            # Конец лимитного ордера
            # ==========================================
